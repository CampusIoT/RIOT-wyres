...
La suite des échanges est sécurisée au moyen de deux clés : la clé de session réseau `NwkSKey` qui sert à signer (et donc autentifié) les messages et la clé de session applicative `AppSKey` qui sert à chiffrer (donc rendre confidentiel) la charge utile (ou applicative) du message.
