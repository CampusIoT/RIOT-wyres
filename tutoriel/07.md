...
> Remarque pour le chapitre 8: nommez l'équipement en le préfixant par la chaîne `WYRES_` : le tableau de bord Grafana est configuré pour n'afficher que les équipements dont le nom commence par `WYRES_`.
...
L'équipement est crée dans l'application qui vous a été affectée à votre groupe.
...
> Remarque: si vous avez le message `Warning: already joined !`, utilisez `loramac erase` et si vous ny' arrivez pas, effacez la totalité de l'EEPROM avec `erase` dans la section [EEPROM du chapitre 3](./03.md#eeprom).
...
> Si vous avez cette erreur `enqueue downlink payload error: create device-queue item error: rpc error: code = InvalidArgument desc = device security-context out of sync (code: 13)`, c'est que le serveur n'a pour l'instant pas recu de premier message montant (uplink). Vérifiez le compteur de frame montant dans l'onglet `Activation`. 
...
Créez un nouvel équipement `WYRES_123_12345679_BALLOON` avec l'dentifiant `DevEUI` et la clé AES128 (`AppKey`) générés et avec le Device Profile `OTAA_CLASS_A_BALLOON` ([codec](https://github.com/CampusIoT/RIOT-wyres/blob/main/apps/field_test_device/codec/decoder_lns.js))
...
## Idée de mini-projet : [Minitel](https://fr.wikipedia.org/wiki/Minitel) 📺 LoRaWAN Class C

L'idée de ce mini-projet est de recycler un minitel (chiné dans une brocante ou sur le Bon Coin) pour chatter via un réseau LoRaWAN.

A partir des exemples précédents, interfacez la carte Wyres par le port UART au port du Minitel 📺 via la carte d'adaptation des niveau de tension 12V0 -> 3V3 pour envoyer des messages dans des paquets LoRaWAN (fport=20) et pour afficher les messsages texte recus.

La carte reliée au minitel doit être enregistrée en classe C avec ce Device Profile `OTAA_CLASS_A_TEXT`. 

En complément, vous pouvez aussi compléter le texte du message à envoyer avec:
* la température (en `°C`) relevée par la carte quand `%c` se trouve dans le message texte à envoyer
* la température (en `°F`) relevée par la carte quand `%f` se trouve dans le message texte à envoyer
* la pression relevée par la carte quand `%p` se trouve dans le message texte à envoyer
* l'orientation relevée par l'accéleromètre de la carte quand `%o` se trouve dans le message texte à envoyer
* la tension de l'alimentation (VBat) relevée par la carte quand `%v` se trouve dans le message texte à envoyer
* le date  de la RTC de la carte au format `YYYY-MM-DD` quand `%D` se trouve dans le message texte à envoyer.
* l'heure de la RTC de la carte au format `HH:MM:SS` quand `%H` se trouve dans le message texte à envoyer. 
* ...
* l'ensemble des mesures quand `%a` se trouve dans le message texte à envoyer

La carte synchronise sa RTC au moyen de la spécification `App Clock Sync`.

`help` affiche la bannière et l'aide

![](images/minitel-wyres-lorawan.jpg)
Crédit photo: Nicolas Picaud

Testez les messages suivant:
```
Nous sommes le %D et il est %H.
La température est de %c °C et la pression atmospherique est de %p hPa.
La tension d'alimentation est %v mV.
```

Ajoutez l'envoi d'un message périodique de "heartbeat" (fport=10) quand le minitel n'a pas été utilisé pendant 15 minutes par un usager. Ce message contient le uptime de la carte, la date de la dernière activité de l'utilisateur, le nombre de messages/caractères échangés, les mesures prises sur la carte (température, pression atmosphérique, luninosité, vbat, orientation ...).

En complément, ajoutez au programme des mesures supplémentaires dans les messages. Ces mesures sont recueillies auprès d'un capteur Grove I2C externe branché sur le connecteur Grove I2C de la carte.

En complément, développez un flot Nodered qui envoie en donwlink un message recu à chaque minitel identifié dans le message recu par des sous-chaine préfixée par `@`. 
...
Enfin, ajoutez des caractères d'échappement qui permettent d'afficher quelques chaines de [code semi-graphique](https://forum.museeminitel.fr/t/codes-semi-graphiques/1344) stockées dans le programme (`const uint8_t smile[] = { ... };`) représentant quelques émojis ou quelques logos (`UGA`, `FabMaSTIC` ...) sur l'écran du minitel ou bien d'afficher des [bannières ASCII Art](https://github.com/codewithnick/ascii-art).

![stum1b](https://jbellue.github.io/stum1b/img/p101/1.svg)


## Idée de mini-projet : [Téléphone à cadran](https://fr.wikipedia.org/wiki/Cadran_t%C3%A9l%C3%A9phonique) 📺 LoRaWAN Class A

Une idée de projet est de recycler un cadran téléphonique pour envoyer des messages courts en LoRaWAN au moyen d'un [téléphone à cadran](https://fr.wikipedia.org/wiki/Cadran_t%C3%A9l%C3%A9phonique).

Les messages peuvent être des chiffres ou des valeurs alphabétiques associés aux chiffres. Par exemple, `S` `O` `S` se compose au moyen de `7` `0` `7` et pour [`L` `O` `R` `A` avec `5` `0` `7` `2`](images/rotary-dial-phone.mov).


![](images/rotary-dial-phone.jpg)
