...
A partir des exemples précédents, interfacez la carte Wyres par le port UART au port du Minitel via la carte d'adaptation des niveaux de tensions 12V0 -> 3V3 pour envoyer des messages dans des paquets LoRaWAN et pour afficher les messsages texte recus.

Vous pouvez aussi compléter le texte avec:
* la temperature relevée par la carte quand `%t` se trouve dans le message texte à envoyer
* la pression relevée par la carte quand `%p` se trouve dans le message texte à envoyer
* l'orientation relevée par la carte quand `%o` se trouve dans le message texte à envoyer
* la tension de l'alimentation (VBat) relevée par la carte quand `%v` se trouve dans le message texte à envoyer
* la RTC de la carte quand `%T` se trouve dans le message texte à envoyer
* ...
