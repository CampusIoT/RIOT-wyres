...
<summary>Réponse</summary>
<ol>
<li>Rédémarrage de la carte avec un aimant: Cette balance magnétique (dans le cercle rouge dans la photo ci-dessous) permet d'actionner cette GPIO au travers d'un boitier étanche. Cela permet de disposer d'un bouton qui permet de réinitiaiser la carte (*reboot*) sans devoir ouvrir le boitier ou ajouter un bouton étanche coûteux. Vous pourrez écrire le traitant d'interruption qui est déclenchée au passage à l'état bas (<code>LOW<c/ode>) de la GPIO <code>PA8</code>code>.</li>
...
<li>Piège à rongeur (Rat trap)</li>
