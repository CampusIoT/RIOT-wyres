...
Les threads permettent de réaliser plusieurs tâches en parallèle par le programme. RIOT offre de mécanismes de barrière de concurrence entre les threads (mutex par exemple) et des moyens de communication par message.
...
La commande `ps` du Shell permet de lister les threads actives dans le programme. `main` correspond à la fonction `main()` qui exécute le Shell et `isr_stack` correspond à la thread thread qui gère les traitements des interuptions, timers ... Il faut souvent ajuster la taille de la pile de `isr_stack` avec `CFLAGS += -DISR_STACKSIZE=2048` par exemple. Idem pour `main` avec `THREAD_STACKSIZE_MAIN`.
...
L'EEPROM est une (petite) mémoire persistante qui est en général utilisée pour des paramêtres de personnalisation ou de configuration de l'équipement, ou sauvegarder des informations de contexte lors du sommeil profond (~deep-sleep~).

Le microcontroleur de la carte Wyres Base a une EEPROM de 8096 octets.
...
> Attention: il faut mieux ne pas stocker des secrets (clés cryptographiques par exemple) dans l'EEPROM : il est préférable d'utiliser la mémoire persistante sécurisée d'un Secure Element (SE) intégré au microcontroleur ou d'un composant externe. Cependant, le MCU de la carte Wyres n'est pas doté d'un tel composant SE.
...
<li>deriver d'autres identifiants (`deveui`, ...)</li>
<li>...</li>
</ul>
</details>
