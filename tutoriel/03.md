...
<details>
<summary>Console (<code>tio</code>)</summary>
<pre>
...
```bash
make CFLAGS+=-DTHREAD_STACKSIZE_DEFAULT=512U flash
```

Que se passe t'il ?

> Les valeurs `THREAD_STACKSIZE_DEFAULT` et `ISR_STACKSIZE` peuvent ajuster pour augmenter la taille des piles des threads et de la thread `isr_stack`.
...
<details>
<summary>Console (<pre>tio</pre>)</summary>
<pre>
